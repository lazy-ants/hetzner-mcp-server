...
**104 tools** across 13 resource domains, with 4 entry points so you can pick the right server for your MCP client's tool limit.
...
| `hetzner-mcp-compute` | Servers, Images, ISOs, Placement Groups, Datacenters/Locations/Server Types | 36 |
| `hetzner-mcp-network` | Networks, Firewalls, Load Balancers, Certificates | 39 |
| `hetzner-mcp-resources` | Volumes, Floating IPs, Primary IPs, SSH Keys | 29 |

Use split servers if your MCP client has a tool limit (e.g., some clients cap at 40 tools).
...
```json
{
  "mcpServers": {
    "hetzner-compute": {
      "command": "npx",
      "args": ["-y", "-p", "hetzner-mcp-server", "hetzner-mcp-compute"],
      "env": { "HETZNER_API_TOKEN": "your-token-here" }
    },
    "hetzner-network": {
      "command": "npx",
      "args": ["-y", "-p", "hetzner-mcp-server", "hetzner-mcp-network"],
      "env": { "HETZNER_API_TOKEN": "your-token-here" }
    },
    "hetzner-resources": {
      "command": "npx",
      "args": ["-y", "-p", "hetzner-mcp-server", "hetzner-mcp-resources"],
      "env": { "HETZNER_API_TOKEN": "your-token-here" }
    }
  }
}
...
### Reference Data (6 tools) — compute
...
### Load Balancers (16 tools) — network
...
### Floating IPs (8 tools) — resources
